const API = "http://localhost:8001/notificaciones";
let skip = 0;
const limit = 20;
...
function createItem(n) {
  ...
  item.addEventListener('click', async (e) => {
    // prevent clicking the inner link toggling double behavior
...
function renderGrouped(data) {
  ...
  // append groups in order of keys (newest first)
...
async function loadMore() {
  ...
  renderGrouped(data);
  // increase skip for pagination
  skip += limit;
}

// mark all visible as read (simple implementation)
...
document.getElementById('verTodo').addEventListener('click', (e) => {
  e.preventDefault();
  // simple behavior: clear and reload from start
